...
d3.json("http://127.0.0.1:5000/api/v1.0/nfl").then(function(nfldata) {
    // Print the attendance nfldata
    console.log(nfldata);
    //console.log(nfldata[1].lat);
    //define map
    var myMap = L.map("map", {
        center: [
          37.09, -95.71
    ...
    for(var i = 0; i<nfldata.length; i++){
        var color = "";
        if (nfldata[i].attendance > 800000 && nfldata[i].attendance < 900000) {
            color = "#fee5d9";
        }
        else if (nfldata[i].attendance > 900000 && nfldata[i].attendance < 1000000) {
            color = "#fcae91";
        }
        else if (nfldata[i].attendance > 1000000 && nfldata[i].attendance < 1100000) {
            color = "#fb6a4a";
        }
        else if (nfldata[i].attendance > 1100000) {
            color = "#cb181d";
        }
        else {
            color = "red";
        ...
        L.circle([nfldata[i].lat, nfldata[i].long], {
            ...
            color: "white",
            fillColor: color,
            radius: nfldata[i].attendance*0.06
        }) .bindPopup(`<h3> ${nfldata[i].team} </h3><hr> <h4> Attendance: ${nfldata[i].attendance} </h4>`)
           .addTo(myMap);
    };
    });
    
    d3.json("http://127.0.0.1:5000/api/v1.0/mlb").then(function(nfldata){
        console.log(nfldata);
